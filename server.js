...
const mockEvents = {
    events: [
        { title: 'TexOps Event Gala', id: 1, description: 'something really cool' },
        { title: 'TexOps Event Number 2', id: 2, description: 'Our Cool Event' },
        {title: 'TexOps Marketing Event', id: 3, description: 'TexOps Test Event'},
        {title: 'TexOps Marketing Event 2', id: 3, description: 'TexOps Test Event 2'}
    ]
};
...
function getEvents(req, res) {
    firestore.collection("texopsevents").get()
        .then((snapshot) => {
            if (!snapshot.empty) {
                const ret = { events: [] };
...
app.post('/event', (req, res) => {
    ...
    const ev = {
        ...
        eventdescr: req.body.desc,
        //location: req.body.location,
        eventlikes: 0
    }
    firestore.collection("texopsevents").add(ev).then(ret => {
...
function changeLikes(req, res, id, increment) {
    ...
    firestore.collection("texopsevents").doc(id).get()
        .then((snapshot) => {
